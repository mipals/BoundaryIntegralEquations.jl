...
function maximum_side_length(element_coordinates)
    return maximum(sqrt.(sum(diff([element_coordinates element_coordinates[:, 1]];
                                  dims=2) .^ 2; dims=1)))
end
...
function minimum_side_length(element_coordinates)
    return minimum(sqrt.(sum(diff([element_coordinates element_coordinates[:, 1]];
                                  dims=2) .^ 2; dims=1)))
end
...
function max_node_distance(coords)
    ...
    @inbounds for i in 2:size(coords, 2)
...
function singular_triangle_integration(shape_function::SurfaceFunction,
    ...
    divsOUT = subdivide_triangle(shape_function, divsIN, source, element_coordinates,
                                 element_size, TolE)
