...
println("Reduced basis size $(size(V,2)) | A reduction in DOF of $(1 - size(V,2)/size(V,1)) %");
...
for (i,k) in enumerate(ks)
    ...
    p1_chief[i] = p_field_chief[1];     # Saving pressure 1
    p2_chief[i] = p_field_chief[2];     # Saving pressure 1
